...
public class ProbeRepository implements IProbeRepository
{
    ...
    public List<Check> getCandidates(Probe probe)
    {
        String sql = ""
                ...
                + "    FROM ( "
                + "    	SELECT candidate.id "
                + "    	FROM \"check\" AS candidate"
                + "     INNER JOIN \"headers\" "
                + "     ON \"check\".id = \"headers\".id "
                + "    	WHERE candidate.disabled IS NULL AND ((:username IS NULL AND candidate.\"internal\" = FALSE) OR (:username = \"headers\".username AND candidate.\"internal\" = TRUE)) AND ((state = 'WAITING'::state AND refresh <= NOW()) OR (state = 'ELECTED'::state AND locked <= NOW())) "
